...
void LCD_initSPI()
{
    ...
    const eUSCI_SPI_MasterConfig spiMasterConfig =
    {
        ...
        12000000, // SPICLK = 500khz
        EUSCI_B_SPI_MSB_FIRST, // MSB First
        EUSCI_B_SPI_PHASE_DATA_CHANGED_ONFIRST_CAPTURED_ON_NEXT, // Phase
        EUSCI_B_SPI_CLOCKPOLARITY_INACTIVITY_HIGH, // High polarity
        EUSCI_B_SPI_3PIN // 3Wire SPI Mode
    };
    /* Enabling port 10 pins 4 and 5 as outputs */
    P10->SEL0 &= ~0x30;
    P10->SEL1 &= ~0x30;
    P10->DIR |= 0x36;
    P10->OUT &= 0x36;
...
void LCD_DrawRectangle(int16_t xStart, int16_t xEnd, int16_t yStart, int16_t yEnd, uint16_t Color)
{
    ...
    LCD_WriteReg(VERT_ADDR_START_POS, xStart+1);
    LCD_WriteReg(VERT_ADDR_END_POS, xEnd);

    LCD_SetPoint(xStart,yStart,Color);//sets cursor and initial pint
...
{
    LCD_DrawRectangle(MIN_SCREEN_X,MAX_SCREEN_X,MIN_SCREEN_Y,MAX_SCREEN_Y,Color);
}
...
Point* TP_ReadXY()
{
    ...
    SPI_CS_TP_LOW;
    SPISendRecvByte(CHX);
    coordinates.x = (SPISendRecvByte(0) << 8);
    coordinates.x |= SPISendRecvByte(0);
    SPISendRecvByte(CHY);
    coordinates.y = (SPISendRecvByte(0) << 8);
    coordinates.y |= SPISendRecvByte(0);
    SPI_CS_TP_HIGH;
    coordinates.x *= 0.01059322033898;
    coordinates.y *= 0.00794491525423;
    coordinates.x -= 30;
    coordinates.y -= 15;
    return &coordinates;
}
